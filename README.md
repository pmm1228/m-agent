...
mAgent 是一个基于 Nuxt 4 + Vue 3 的本地 AI 工作区示例项目，集成了 AI 对话、会话管理和待办事项管理。项目使用 SQLite 持久化数据，默认接入智谱 GLM 模型，整体界面采用 macOS 风格的工作区体验。
...
| `NUXT_ZHIPU_API_KEY` | AI 对话必填 | 空 | 智谱 API Key。未配置时，普通 AI 回复会失败并显示可重试状态。 |
